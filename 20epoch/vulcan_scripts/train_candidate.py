...
train_candidate.py  --  Stage 1: Candidate-only baseline
  - support_ratio = 0.0  (all global fallback candidates)
  - semantic features zeroed out
  - candidate_valid_mask = all 1s (no hard constraint)
  - best_candidate_idx = argmax(candidate_ious) directly
  - NO masked_fill on scores in training loop
Saves: 01_candidate_last.pt / 01_candidate_best.pt
"""
...
class BootplaceSupportSurfaceDataset(Dataset):
    def __init__(
        ...
        support_ratio=0.0,          # <<< KEY DIFF: all global fallback
        base_seed=42,
    ):
    ...
    def __getitem__(self, idx):
        ...
        # 7. candidates  (support_ratio=0.0 => all global fallback)
        candidate_boxes, support_mask = sample_support_surface_candidates(
            ...
            support_ratio=self.support_ratio,     # 0.0
            seed=self.base_seed + idx,
        )

        num_candidates = candidate_boxes.shape[0]

        # <<< KEY DIFF: zeroed semantic features
        candidate_sem_feats = torch.zeros((num_candidates, 4), dtype=torch.float32)

        # <<< KEY DIFF: all-ones valid mask (no filtering)
        candidate_valid_mask = torch.ones(num_candidates, dtype=torch.float32)

        # 9. IoU supervision
        candidate_ious = pairwise_iou(candidate_boxes, target_bbox.unsqueeze(0)).squeeze(1)

        # <<< KEY DIFF: argmax directly, no valid-mask filtering
        best_candidate_idx = torch.argmax(candidate_ious)
...
# ───────────────────────────── training loop (CANDIDATE VARIANT) ────
...
def run_one_epoch_support_surface(model, loader, optimizer=None, device="cuda"):
    ...
    for batch in pbar:
        ...
        with torch.set_grad_enabled(is_train):
            ...
            # <<< KEY DIFF: NO masked_fill (mask is all 1s anyway)
...
def main():
    parser = argparse.ArgumentParser(description="Train candidate-only baseline")
    parser.add_argument("--data_root", type=str,
                        default="/scratch/zchai3/ece740_placement/bootplace_like_data")
    parser.add_argument("--cityscapes_root", type=str,
                        default="/scratch/zchai3/ece740_final__runtime/datasets__raw/cityscapes/data")
    ...
    # Pre-cache ResNet-18 weights (compute nodes may lack internet)
    import os
    try:
        from torchvision.models import resnet18, ResNet18_Weights
        ...
        raise RuntimeError(
            f"Failed to load ResNet-18 weights. If on a compute node without internet, "
            f"run this on the login node first:\n"
            f"  python -c \"from torchvision.models import resnet18, ResNet18_Weights; "
            f"resnet18(weights=ResNet18_Weights.DEFAULT)\"\n"
    ...
    train_ann_path = Path(args.data_root) / "train" / "annotations_train.jsonl"
    train_records = read_jsonl(train_ann_path)
    ...
    train_loader = DataLoader(
        ...
        num_workers=4, collate_fn=support_surface_collate_fn, pin_memory=True,
    )
    ...
    last_ckpt = save_dir / "01_candidate_last.pt"
    best_ckpt = save_dir / "01_candidate_best.pt"
