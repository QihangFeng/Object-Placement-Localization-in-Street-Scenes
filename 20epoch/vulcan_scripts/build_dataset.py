#!/usr/bin/env python3
"""
build_dataset.py  --  Generate bootplace_like_data from Cityscapes raw data.

Steps:
  1. Scan instance maps → extract object bounding boxes and RGBA patches → save index
  2. For each target object, inpaint it from the image → save background + annotations

Usage on Vulcan:
  python build_dataset.py \
      --cityscapes_root /scratch/zchai3/ece740_final__runtime/datasets__raw/cityscapes/data \
      --out_root /scratch/zchai3/ece740_placement/bootplace_like_data
"""
...
try:
    ...
    raise ImportError(
        "opencv-python-headless is required but not installed.\n"
        "Install it with:\n"
        "  /scratch/zchai3/ece740_final__runtime/env__vlm_base/bin/pip install opencv-python-headless"
    )
import numpy as np
